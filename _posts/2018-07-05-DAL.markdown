...
## Why Discriminative Active Learning May Be Promising
...
We can see that our method starts out well and beats random sampling, but as the experiment progresses we see that it improves more and more slowly until it reaches accuracies that are the same as random sampling... We're also nowhere close to uncertainty sampling, but let's put that aside for now.

So our method isn't behaving like we expected. This means we're not understanding something basic about it... We choose to make things as simple as possible in order to understand what's going on, and so we look at how our method behaves on synthetic 2-dimensional data. Like we did before, we'll make two blobs that are linearly seperable and look at what examples our method chooses:
...
This sort of data is very different from real data. We can't really picture high dimensional data manifolds and our lower dimensional intuitions often fail in high dimensions. still, we can learn a thing or two from looking at toy problems.



- move to synthetic datasets
- outlier issue and mode covering
...
[thirs post]: https://dsgissin.github.io/DiscriminativeActiveLearning/2018/07/05/Experiments.html
