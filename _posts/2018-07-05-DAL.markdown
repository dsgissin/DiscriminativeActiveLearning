...
Next, we move to comparing our method to all the other ones on MNIST and CIFAR-10. These plots are the ones presented in our final [paper]:
...
We compared DAL to the methods we covered in the [previous post][third post] and saw that it performs similarly to the core set approach, while being easier to implement and more scalable. We also saw that our method shares the task-agnostic nature of the core set approach, making it very easy to adapt to machine learning tasks which aren't classification. While our method seems to do a great job, it isn't clearly better than any of the current baselines and so we can't recommend using it without further experiments.

TODO: if they want to delve deeper, direct them to the DAL paper...
