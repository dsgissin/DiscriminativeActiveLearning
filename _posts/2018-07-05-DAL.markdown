...
Next, we move to comparing our method to all of the other ones on MNIST and CIFAR-10:

{% include image.html path="DAL/mnist_all.png" %}

{% include image.html path="DAL/cifar10_all.png" %}

In the MNIST results we see more evidence that our method behaves in a similar way to the core set approach, since their results are more or less identical. This means that on the MNIST dataset we see clear different results between the uncertainty based methods (including the adversarial approach) and the covering based methods (core set and discriminative).

On the CIFAR-10 front we see that the results of our method are in the same ballpark as the other methods, while still being inconclusive and requiring more experiments to reduce the variance of the results.

Finally, let's look at how our method looks in the label entropy metric againt the other methods:
